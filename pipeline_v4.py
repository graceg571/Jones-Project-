...
import argparse 
import re 
from pyben import PyBenEncoder, PyBenDecoder
...
# GENERATING DISTRICT HELPER FUNCTIONS
...
# CREATING SETTING FILES HELPER FUNCTIONS 
...
def _create_settings_files(config):
    ...
    for dc in config["districting_configs"]:
        path_to_districting = config["plans_folder"] / f"{run_name}_{dc['num_districts']}_districts.jsonl.ben"
        all_plans = []
        for assignment in PyBenDecoder(path_to_districting):
        ...
        for plan_idx, assignment in tqdm(enumerate(all_plans), total=len(all_plans), desc=f"Creating settings files for each district of {dc['num_districts']}-district plans"):
            ...
            for district_label, row in district_demos.iterrows():
                ...
                settings = {
                    ...
                    "num_reps" : max_plans / (config["chain_length"] * dc["num_districts"])
...
def _generate_preference_profiles(config):
    ...
    for dc in config["districting_configs"]:
        print(f"starting config {dc['num_districts']} x {dc['num_winners_per_district']}")
        num_reps = dc["num_reps"]
        for rep in range(num_reps):
            for generative_mode in ['slate-pl', 'name-cumulative']:
                settings_folder = Path(f"outputs/{run_name}/settings/{dc['num_districts']}_districts")
                for settings_path in settings_folder.glob("*.json"):#erative_mode} model for each district plan of {dc['num_districts']}-district plans"): # loop through all plans and all districts 
...
if __name__ == "__main__":
    ...
    for filepath in dc_paths.values():
        filepath.mkdir(parents=True, exist_ok=True)
    config["plans_folder"] = dc_paths["plans_folder"]
    # calculate number of district plans per configuration
    max_plans = 0
    for dc in config["districting_configs"]:
        num_plans = dc["num_districts"] * config["chain_length"]
        if num_plans > max_plans:
            max_plans = num_plans
    # Calculate the number of preference profile samples per districting configuration so they're the same
