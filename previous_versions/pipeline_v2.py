...
def _create_settings_files(config):
    ...
    for dc in config["districting_configs"]:
        ...
        for plan in tqdm(all_plans, total=len(all_plans), desc=f"Creating settings files for each district of {dc['num_districts']}-district plans"):
...
def _simulate_elections(config):
    ...
    for generative_mode in ['slate-pl', 'name-cumulative']:
        ...
        for dc in config["districting_configs"]:
            ...
            plan_dict = {}
            profile_files_paths = []
            for profile_file in tqdm(profile_files.glob("*.csv"), total = len(list(profile_files.glob("*.csv"))), desc=f"Simulating election results for profiles generated with {generative_mode} model for each district plan of {dc['num_districts']}-district plans"):
                ...
                match = re.search(r'_(\d+)_districts_profile_sample_(\d+)_district_(\d+)_rep_v(\d+)', filename)
                plan_index = int(match.group(2))
                district = int(match.group(3))
                pp_sample = int(match.group(4))
                plan_key = (plan_index, pp_sample)
                if plan_key not in plan_dict:
                    plan_dict[plan_key] = {"plan_index" : plan_index, "preference_profile_sample" : pp_sample, "districts" : []}
                ...
                plan_dict[plan_key]["districts"].append(plan_results)
